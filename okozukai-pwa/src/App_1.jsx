          ...
          <div style={{ background: "#FBFAFF", border: `2px solid ${C.plum}22`, borderRadius: 22, padding: 16, marginBottom: 14 }}>
            ...
            {AFFILIATE.map((a) => (
              <div key={a.title} style={{ display: "flex", alignItems: "center", gap: 10, background: "#fff", borderRadius: 14, padding: "10px 12px", marginBottom: 8, boxShadow: "0 2px 8px rgba(58,46,92,.05)" }}>
                ...
                <a href={a.url || "#"} onClick={(e) => { if (a.url) { e.preventDefault(); window.open(a.url, "_blank", "noopener"); } }} target="_blank" rel="noopener sponsored" style={{ textDecoration: "none", background: a.url ? C.violet + "1A" : "#EEE", color: a.url ? C.violet : "#AAA", fontWeight: 700, fontSize: 12, borderRadius: 10, padding: "7px 12px", flexShrink: 0, cursor: a.url ? "pointer" : "default" }}>みる</a>
              </div>
            ))}
